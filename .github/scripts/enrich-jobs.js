  ...
  for (const job of batch) {
    ...
    if (job.source === 'workday') {
      ...
      try {
        ...
        // OUT-14: Extract additional locations for multi-location routing
        const addLocs = data?.jobPostingInfo?.additionalLocations;
        if (Array.isArray(addLocs) && addLocs.length > 0) {
    ...
    if (rawHtml) {
      ...
      if (text.length > 20) {
        ...
        const entry = { id: job.id, description_text: text };
        // OUT-14: Store additional locations for multi-location routing
        if (additionalLocations) entry.additional_locations = additionalLocations;
...
function loadEnrichedIds() {
  ...
  for (const line of lines) {
    try {
      ...
      if (obj.id && (obj.enricher_version || 0) >= ENRICHER_VERSION) ids.add(obj.id);
    } catch (_) {}
  }
  return ids;
}
