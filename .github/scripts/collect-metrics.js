...
function getEnrichmentStats() {
  try {
    ...
    const tierTotal = (tiers.t0 || 0) + (tiers.t1 || 0) + (tiers.t2 || 0) + (tiers.t3 || 0);
    return {
      ...
      enricherVersion: stats.enricher_version ?? null,
      tiers: {
        t0: tiers.t0 ?? 0,
        t1: tiers.t1 ?? 0,
        t2: tiers.t2 ?? 0,
        t3: tiers.t3 ?? 0,
        t3Rate: tierTotal > 0 ? Math.round(100 * (tiers.t3 || 0) / tierTotal) / 100 : null,
      },
      descWaiting: stats.desc_waiting ?? 0,
      reenrichmentPending: stats.reenrichment_pending ?? 0,
    };
  } catch {
    return null;
  }
