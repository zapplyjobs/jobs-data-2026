  ...
  if (!fs.existsSync(metadataPath)) {
    ...
    if (fs.existsSync(enrichStatsPath)) {
      try {
        ...
        // Check 15b: Per-source T3 regression — catch source-specific drops masked by aggregate
        const STRUCTURALLY_LOW_T3 = new Set(['simplify', 'apple', 'google', 'jsearch', 'eightfold', 'amd']);
        const tiersBySource = enrichStats.tiers_by_source || {};
        for (const [src, tiers] of Object.entries(tiersBySource)) {
          const total = (tiers.t0 || 0) + (tiers.t1 || 0) + (tiers.t2 || 0) + (tiers.t3 || 0);
          if (total < 20) continue;
          if (STRUCTURALLY_LOW_T3.has(src)) continue;
          const t3Rate = (tiers.t3 || 0) / total;
          if (t3Rate < 0.50) {
            failures.push(`**Enrichment T3 regression (${src})**: ${Math.round(t3Rate * 100)}% T3 (${tiers.t3}/${total}, expected >60%) — source-specific enrichment may have broken`);
          }
        }
