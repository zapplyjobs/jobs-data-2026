...
function summarizeEnrichmentVisaContext(enrichStats) {
  const bySource = enrichStats?.by_source || {};
  const reasonTotals = {};
  const unresolvedBySource = {};
  for (const [source, row] of Object.entries(bySource)) {
    const unresolved = row?.no_visa_signal || 0;
    if (unresolved > 0) unresolvedBySource[source] = unresolved;
    const reasonCounts = row?.visa_reason_counts || {};
  ...
  return {
    top_unresolved_reasons: Object.entries(reasonTotals)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([reason, count]) => ({ reason, count })),
    top_unresolved_sources: Object.entries(unresolvedBySource)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
...
async function runChecks() {
  ...
  if (!fs.existsSync(metadataPath)) {
    ...
    if (fs.existsSync(enrichStatsPath)) {
      try {
        ...
        enrichmentContext = summarizeEnrichmentVisaContext(enrichStats);
  ...
  return { failures, enrichmentContext };
}

async function postAlert(failures, enrichmentContext) {
  ...
  const fields = [];
  const topReasons = enrichmentContext?.top_unresolved_reasons || [];
  if (topReasons.length > 0) {
    fields.push({
      name: 'ENR unresolved reasons',
      value: topReasons.map(({ reason, count }) => `${reason}: ${count}`).join('\n').slice(0, 1024),
      inline: false,
    });
  }
  const topSources = enrichmentContext?.top_unresolved_sources || [];
  if (topSources.length > 0) {
    fields.push({
      name: 'ENR unresolved sources',
      value: topSources.map(({ source, count }) => `${source}: ${count}`).join('\n').slice(0, 1024),
      inline: false,
    });
  }
...
async function main() {
  ...
  const { failures, enrichmentContext } = await runChecks();
  ...
  await postAlert(failures, enrichmentContext);
